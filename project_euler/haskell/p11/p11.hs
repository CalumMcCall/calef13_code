...
--Calculates the maximum value for a row. Using transpose, 
--we can also use this function for the columns
maxRows :: (Num a, Ord a) => [[a]] -> a
maxRows []     = 0
maxRows [[]]   = 0
maxRows num@(x:xs) = (maximum (map maxRow num))

maxRow :: (Num a, Ord a) => [a] -> a
maxRow []           = 0
maxRow (w:[])       = 0
maxRow (w:x:[])     = 0
maxRow (w:x:y:[])   = 0
maxRow (w:x:y:z:xs) = max curSum nextSum
                    where curSum  = w*x*y*z
                          nextSum = maxRow (x:y:z:xs)

maxForwDiagonals :: (Num a, Ord a) => [[a]] -> a
maxForwDiagonals []             = 0
maxForwDiagonals (w:[])         = 0
...
maxBackDiagonals :: (Num a, Ord a) => [[a]] -> a
maxBackDiagonals []             = 0
maxBackDiagonals (w:[])         = 0
