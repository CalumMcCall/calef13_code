import List
nums = [[3,7,1,0,7,2,8,7,5,3,3,9,0,2,1,0,2,7,9,8,7,9,7,9,9,8,2,2,0,8,3,7,5,9,0,2,4,6,5,1,0,1,3,5,7,4,0,2,5,0],
...
main = do
       let powersTen = map (10^) [0,1..]
       --transposing the array allows us to more easily sum it
       let sumNums = zipWith (*) powersTen (map sum (transpose nums))
       putStrLn $ take 10 (show (sum sumNums))
